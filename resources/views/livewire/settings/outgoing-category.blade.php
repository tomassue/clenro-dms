<div>
                ...
                <div class="card-body d-flex flex-column">
                    <div class="row g-5 justify-content-between">
                        ...
                        <div class="col-sm-12 col-md-12 col-lg-2 text-end">
                            <button type="button" class="btn btn-primary" wire:click="$dispatch('show-outgoingCategoryModal')">Add Category</button>
                        </div>
                    </div>
                    <div class="mt-5">
                        <div class="table-responsive">
                            <table class="table table-rounded table-striped border gy-7 gs-7 align-middle">
                                ...
                                <tbody>
                                    @forelse($outgoing_categories as $index=>$item)
                                    <tr>
                                        ...
                                        <td>
                                            <a type="button" style="white-space: nowrap;" class="btn btn-sm btn-secondary me-2 mb-2" wire:click="readOutgoingCategory({{ $item->id }})">Edit</a>
                                            <a type="button" style="white-space: nowrap;" class="btn btn-sm {{ $item->deleted_at ? 'btn-info' : 'btn-danger' }} mb-2" wire:click="{{ $item->deleted_at ? 'restoreOutgoingCategory(' . $item->id . ')' : 'deleteOutgoingCategory(' . $item->id . ')' }}">{{ $item->deleted_at ? 'Activate' : 'Deactivate' }}</a>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
    ...
    @include('livewire.settings.modals.outgoing-category-modal')
</div>

@script
<script>
    $wire.on('show-outgoingCategoryModal', () => {
        $('#outgoingCategoryModal').modal('show');
    });

    $wire.on('hide-outgoingCategoryModal', () => {
